...
class AllDataPreprocessor(object):
	...
	def __addExpressionInformation(self, directoryFolder, forTraining=False):
		...
		if not rnaseq.empty:
			rnaseqData = PatientData("RNASeq", pd.Series(data=rnaseqClinical.index.values, name="Patient",
														 index=rnaseqClinical.index.values))
			rnaseqData.set_ages(rnaseqClinical.loc[rnaseqClinical.index, "D_Age"].copy())
			rageRiskDF = rnaseqClinical.loc[rnaseqClinical.index, "D_Age"].copy()
			rageRiskDF.name = "D_Age_Risk"
			rageRiskDF[rageRiskDF >= 65] = 1
			rageRiskDF[rageRiskDF < 65] = 0
			rnaseqData.set_ageRisk(rageRiskDF)
			rnaseqData.set_ISSs(rnaseqClinical.loc[rnaseqClinical.index,"D_ISS"].copy())
			if forTraining:
		...
		if not microarrays.empty:
			microarraysData = PatientData("MicroArrays",
										  pd.Series(data=microarraysClinical.index.values, name="Patient",
													index=microarraysClinical.index.values))
			microarraysData.set_ages(microarraysClinical.loc[microarraysClinical.index, "D_Age"].copy())
			mageRiskDF = microarraysClinical.loc[microarraysClinical.index, "D_Age"].copy()
			mageRiskDF.name = "D_Age_Risk"
			mageRiskDF[mageRiskDF >= 65] = 1
			mageRiskDF[mageRiskDF < 65] = 0
			microarraysData.set_ageRisk(mageRiskDF)
			microarraysData.set_ISSs(microarraysClinical.loc[microarraysClinical.index,"D_ISS"].copy())
			if forTraining:
	...
	def getPatientDataByDataset(self, directoryFolder='/test-data/', useFiltered=True, forTraining=False,
								groupAges=False):
	...
	def prepareDatasetForStacking(self, datasets, useFiltering=True):
		...
		for dataset in datasets.values():
			...
			if genes_scoring is None:
				...
				genes_scoring = pd.concat(
					[genes_scoring, self.__addDatasetNameToDataframe(dataset.get_genes_scoring(), dataset)], axis=1)

			if genes_function_associated is None:
				genes_function_associated = self.__addDatasetNameToDataframe(dataset.get_genes_function_associated(),
																			 dataset)
			else:
				genes_function_associated = pd.concat([genes_function_associated, self.__addDatasetNameToDataframe(
					dataset.get_genes_function_associated(), dataset)], axis=1)
			...
			if big_qss is None:
				...
				big_qss = pd.concat([big_qss, self.__addDatasetNameToDataframe(dataset.get_genes_big_qss(), dataset)],
									axis=1)
			...
			if clustered is None:
				...
				clustered = pd.concat(
					[clustered, self.__addDatasetNameToDataframe(dataset.get_genes_clustered(), dataset)], axis=1)
			...
			if germline is None:
				...
				germline = pd.concat(
					[germline, self.__addDatasetNameToDataframe(dataset.get_genes_germline_risk(), dataset)], axis=1)
			...
			if somaticrisk is None:
				...
				somaticrisk = pd.concat(
					[somaticrisk, self.__addDatasetNameToDataframe(dataset.get_genes_somatic_risk(), dataset)], axis=1)
	...
	def joinDatasetsToSingleDataset(self, datasets, useFiltering=True):
		...
		for dataset in datasets.values():
			...
			if genes_function_associated is None:
				...
				genes_function_associated = pd.concat(
					[genes_function_associated, dataset.get_genes_function_associated()], axis=1)
