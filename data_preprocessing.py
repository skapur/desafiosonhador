...
class MMChallengeData(object):
	...
	def getData(self, datype, level, clinicalVariables=["D_Age", "D_ISS"], outputVariable="HR_FLAG",
		...
		print(str(len(dfiles)), "for type", type_level_sid)
		...
		if len(dframes) > 1:
			...
			print('No datasets found for', type_level_sid)
			empty = pd.DataFrame()
	...
	def getDataDict(self, clinicalVariables=["D_Age", "D_ISS"], outputVariable="HR_FLAG", directoryFolder='/test-data/',
		...
		return {(datype, level): self.getData(
			...
			NARemove,
			None if colParseFunDict is None else colParseFunDict[(datype, level)] if (datype,
																					  level) in colParseFunDict.keys() else lambda
				x: x)
